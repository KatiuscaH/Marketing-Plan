...
class Acompetencia extends Component {
  state = {
...
componentDidMount(){
  ...
  .then(res=>{
    const {presentacion,historia,pest,porter,coatrop,clientes,competencia,proveedores,bcg,dofa,mefi,ansoff} = res.data;
     ...
     if (res.data.competencia) {
      this.setState({ editorState: EditorState.createWithContent(convertFromRaw(JSON.parse( res.data.competencia))) })
  } else {
