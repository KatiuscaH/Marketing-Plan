...
class Aporveedores extends Component {
  state = {
...
componentDidMount(){
  ...
  .then(res=>{
    const {presentacion,historia,pest,porter,coatrop,clientes,competencia,proveedores,bcg,dofa,mefi,ansoff} = res.data;
     ...
     if (res.data.proveedores) {
      this.setState({ editorState: EditorState.createWithContent(convertFromRaw(JSON.parse( res.data.proveedores))) })
  } else {
