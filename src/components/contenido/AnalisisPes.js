...
import { ADD_PEST, ELIMINAR_DATOS_INICIALES_PLAN } from '../../config';
...
class AnalisisPes extends Component {
    ...
    save = () => {
        ...
        axios.put(ADD_PEST.replace(":id", campo),
            { pest: JSON.stringify(this.state.convertedContent) },
            { headers: { Authorization: `Bearer ${localStorage.getItem('id_token')}` } })
            .then((res => {
    ...
    render() {
        ...
        return (
            <div>
                {
                    cargando ? <div>
                        <h1 style={{ textAlign: 'center', color: 'black'  }}>Análisis PEST</h1>
